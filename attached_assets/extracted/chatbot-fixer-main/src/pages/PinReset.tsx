...
export default function PinReset() {
  ...
  const requestOtp = async () => {
    setSending(true);
    const { data, error } = await pinApi.resetRequest();
    setSending(false);
  ...
  const submitNewPin = async (finalConfirm: string) => {
    setSubmitting(true);
    const { error } = await pinApi.resetConfirm(otp, pin, finalConfirm);
    setSubmitting(false);
    if (error) {
      toast.error(error);
      // Backend may have invalidated OTP after consumed: send back to request.
  ...
  return (
    <div className="container py-10 sm:py-16 grid place-items-center">
      <div className="glass-card w-full max-w-md p-6 sm:p-8">
        ...
        <h1 className="mt-6 font-display text-2xl sm:text-3xl">Reset your PIN</h1>
        <p className="text-sm text-muted-foreground mt-1">
          ...
          {step === "newPin"  && "Choose a new 4-digit PIN."}
        ...
        {step === "request" && (
          <div className="mt-8">
            ...
            <Link to="/pin-unlock" className="block mt-2"><Button variant="ghost" className="w-full">Back to PIN entry</Button></Link>
          </div>
        )}
        ...
        {step === "newPin" && (
          <div className="mt-8">
            <PinPad
              value={pin}
              onChange={setPin}
              onComplete={(v) => { if (v.length === 4) { setStep("confirmPin"); setConfirm(""); } }}
            />
          </div>
        )}
        ...
        {step === "confirmPin" && (
          <div className="mt-8">
            <PinPad
              ...
              onChange={setConfirm}
              disabled={submitting}
              onComplete={(v) => v.length === 4 && submitNewPin(v)}
            />
